...
(this repo resolves to project `derevo`). There is no directory layout to maintain — you search and
recall by query.
...
- Keep `title` short and searchable (e.g. `Derevo architecture plan`, `Fixed N+1 in habit summary`).
...
- For knowledge that evolves over time, pass a stable `topic_key` (e.g. `architecture/habits-service`) so the entry is upserted instead of duplicated.
...
Derevo is a fast, native habit tracker — local-first, no accounts, no telemetry.
Privacy is inherent (all data stays on device), but without the heavy encryption
layer of a finance app. Keep things simple and offline.

## Project Structure and Module Organization
Service-oriented layered architecture. No God Object, no redundant wrappers:

`src/` (Rust — business logic crate)
```
src/
├── lib.rs              # Crate exports
├── models.rs           # All domain models (Habit, HabitLog, StreakReward, Goal, ...)
├── error.rs            # DbError + AppError (shared, thiserror)
├── analytics.rs        # HabitAnalytics types (WeekdayEfficiency, MonthlyTrendPoint, ...)
├── db/
...
```
src-tauri/
├── Cargo.toml          # Tauri crate with derevo dependency
├── tauri.conf.json     # Tauri config (window, CSP, frontend path)
├── capabilities/       # Tauri permission definitions
├── src/
│   ├── main.rs         # Tauri entry point
│   ├── lib.rs          # Tauri builder + command registration
│   └── commands.rs     # All #[tauri::command] functions (habits + settings)
└── build.rs
```
...
- Git-flow style: `feat:`, `fix:`, `docs:`, `chore:`, etc.
  Example: `feat(habits): add streak reward creation`.

## Security Notes
- **Local-first**: all data stays on device. No network calls.
- **Input validation**: validate all user input at the service layer.
- **No secrets in git**: API keys, tokens, passwords → env vars or config files outside the repo.
- **Plain SQLite**: no encryption needed for habits data.
