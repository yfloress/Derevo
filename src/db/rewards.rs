...
impl super::Database {
    ...
    pub fn create_streak_reward(&self, reward: &StreakReward) -> Result<(), DbError> {
        ...
        conn.execute(
            "INSERT INTO streak_rewards (id, habit_id, is_consecutive, target_days, target_total, created_at)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
            params![&reward.id, &reward.habit_id, reward.is_consecutive as i32, &reward.target_days, &reward.target_total, &reward.created_at],
        )?;
        Ok(())
    }
    ...
    pub fn get_streak_reward(&self, id: &str) -> Result<Option<StreakReward>, DbError> {
        let conn = self.read()?;
        Self::get_streak_reward_on(&conn, id)
    }

    pub(crate) fn get_streak_reward_on(
        conn: &Connection,
        id: &str,
    ) -> Result<Option<StreakReward>, DbError> {
    ...
    pub fn delete_streak_reward(&self, id: &str) -> Result<(), DbError> {
        let conn = self.write();
        conn.execute("DELETE FROM streak_rewards WHERE id = ?1", params![id])?;
        Ok(())
    }
    ...
    pub fn create_milestone(&self, milestone: &Milestone) -> Result<(), DbError> {
        Self::create_milestone_on(&self.write(), milestone)
    }

    pub(crate) fn create_milestone_on(
        conn: &Connection,
        milestone: &Milestone,
    ) -> Result<(), DbError> {
        conn.execute(
            "INSERT INTO milestones (id, reward_id, target_days, reward_text, unlocked, unlocked_at)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
            params![&milestone.id, &milestone.reward_id, milestone.target_days, &milestone.reward_text, milestone.unlocked as i32, &milestone.unlocked_at],
        )?;
        Ok(())
    }
    ...
    pub fn get_milestones(&self, reward_id: &str) -> Result<Vec<Milestone>, DbError> {
        let conn = self.read()?;
        Self::get_milestones_on(&conn, reward_id)
    }

    pub(crate) fn get_milestones_on(
        conn: &Connection,
        reward_id: &str,
    ...
    pub fn update_milestone(&self, milestone: &Milestone) -> Result<(), DbError> {
        Self::update_milestone_on(&self.write(), milestone)
    }

    pub(crate) fn update_milestone_on(
        conn: &Connection,
        milestone: &Milestone,
    ) -> Result<(), DbError> {
        conn.execute(
    ...
    pub fn create_goal(&self, goal: &Goal) -> Result<(), DbError> {
        ...
        conn.execute(
            ...
            params![&goal.id, &goal.name, &goal.description, &goal.reward_text, &goal.deadline, goal.is_completed as i32, &goal.completed_at, &goal.created_at, goal.archived as i32],
        )?;
        Ok(())
    }
    ...
    pub fn get_goal(&self, id: &str) -> Result<Option<Goal>, DbError> {
        let conn = self.read()?;
        Self::get_goal_on(&conn, id)
    }

    pub(crate) fn get_goal_on(conn: &Connection, id: &str) -> Result<Option<Goal>, DbError> {
        let result = conn.query_row(
            "SELECT id, name, description, reward_text, deadline, is_completed, completed_at, created_at, archived
    ...
    pub fn update_goal(&self, goal: &Goal) -> Result<(), DbError> {
        Self::update_goal_on(&self.write(), goal)
    }

    pub(crate) fn update_goal_on(conn: &Connection, goal: &Goal) -> Result<(), DbError> {
        conn.execute(
    ...
    pub fn archive_goal(&self, id: &str) -> Result<(), DbError> {
        let conn = self.write();
        conn.execute("UPDATE goals SET archived = 1 WHERE id = ?1", params![id])?;
        Ok(())
    }
    ...
    pub fn create_checkpoint(&self, checkpoint: &Checkpoint) -> Result<(), DbError> {
        Self::create_checkpoint_on(&self.write(), checkpoint)
    }

    pub(crate) fn create_checkpoint_on(
        conn: &Connection,
        ...
        conn.execute(
            "INSERT INTO checkpoints (id, goal_id, description, completed, completed_at, sort_order)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
            params![&checkpoint.id, &checkpoint.goal_id, &checkpoint.description, checkpoint.completed as i32, &checkpoint.completed_at, checkpoint.sort_order],
        )?;
        Ok(())
    }
    ...
    pub fn get_checkpoints(&self, goal_id: &str) -> Result<Vec<Checkpoint>, DbError> {
        let conn = self.read()?;
        Self::get_checkpoints_on(&conn, goal_id)
    }

    pub(crate) fn get_checkpoints_on(
        conn: &Connection,
        goal_id: &str,
    ...
    pub fn get_checkpoint(&self, id: &str) -> Result<Option<Checkpoint>, DbError> {
        let conn = self.read()?;
        Self::get_checkpoint_on(&conn, id)
    }

    pub(crate) fn get_checkpoint_on(
        conn: &Connection,
        id: &str,
    ...
    pub fn update_checkpoint(&self, checkpoint: &Checkpoint) -> Result<(), DbError> {
        Self::update_checkpoint_on(&self.write(), checkpoint)
    }

    pub(crate) fn update_checkpoint_on(
        conn: &Connection,
        checkpoint: &Checkpoint,
    ) -> Result<(), DbError> {
        conn.execute(
            "UPDATE checkpoints SET description = ?1, completed = ?2, completed_at = ?3, sort_order = ?4 WHERE id = ?5",
            params![&checkpoint.description, checkpoint.completed as i32, &checkpoint.completed_at, checkpoint.sort_order, &checkpoint.id],
        )?;
        Ok(())
    }
    ...
    pub fn get_checkpoint_counts(&self, goal_id: &str) -> Result<(i32, i32), DbError> {
        ...
        let (total, completed): (i32, i32) = conn.query_row(
            "SELECT COUNT(*), SUM(CASE WHEN completed = 1 THEN 1 ELSE 0 END) FROM checkpoints WHERE goal_id = ?1",
            params![goal_id],
    ...
    pub fn delete_checkpoint(&self, id: &str) -> Result<(), DbError> {
        Self::delete_checkpoint_on(&self.write(), id)
    }

    pub(crate) fn delete_checkpoint_on(conn: &Connection, id: &str) -> Result<(), DbError> {
    ...
    pub fn create_achievement(&self, achievement: &Achievement) -> Result<(), DbError> {
        ...
        conn.execute(
            ...
            params![&achievement.id, &achievement.title, &achievement.description, &achievement.icon_path, &achievement.achievement_type, &achievement.source_id, &achievement.achieved_at],
        )?;
        Ok(())
    }
    ...
    pub(crate) fn get_streak_progress_on(
        conn: &Connection,
        habit_id: &str,
        is_consecutive: bool,
        target_total: Option<i32>,
        ...
        if is_consecutive {
            ...
            Self::calculate_accumulative_progress_on(conn, habit_id, target_total.unwrap_or(30))
        }
    }
    ...
    fn calculate_accumulative_progress(&self, habit_id: &str, days: i32) -> Result<i32, DbError> {
        let conn = self.read()?;
        Self::calculate_accumulative_progress_on(&conn, habit_id, days)
    }

    fn calculate_accumulative_progress_on(
        conn: &Connection,
        habit_id: &str,
        days: i32,
    ) -> Result<i32, DbError> {
        ...
        let count: i32 = conn.query_row(
            "SELECT COUNT(*) FROM habit_logs WHERE habit_id = ?1 AND completed_date >= ?2 AND completed_date <= ?3",
            params![habit_id, start.format("%Y-%m-%d").to_string(), today.format("%Y-%m-%d").to_string()],
            |row| row.get(0),
        )?;
        Ok(count)
    }
