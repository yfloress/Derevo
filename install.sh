...
# Derevo install script
# Builds from source (frontend + release binary) and installs Derevo.
#
# Usage:
...
APP_NAME="derevo"                              # installed command / icon / .desktop name
...
build_app() {
    ...
    log "Building Derevo (frontend + release binary)..."
...
if [ "$DO_UNINSTALL" -eq 1 ]; then
    log "Uninstalling Derevo from ${PREFIX}..."
    uninstall_files
    [ "$IS_MACOS" -eq 0 ] && update_caches
        info "App data (your database in ~/.local/share/Derevo) was left untouched."
    exit 0
fi

if [ "$IS_USER" -eq 1 ]; then
    log "Installing Derevo for current user (${PREFIX})..."
else
    log "Installing Derevo system-wide (${PREFIX})..."
fi
...
log "Derevo installed successfully."
if [ "$IS_MACOS" -eq 1 ]; then
    info "Run 'derevo' from your terminal."
else
    info "Run 'derevo' from your terminal, or find it in your application menu."
fi

if [ "$IS_USER" -eq 1 ]; then
    case ":${PATH}:" in
        *":${HOME}/.local/bin:"*) ;;
        *) warn "${HOME}/.local/bin is not on your PATH; add it to use the 'derevo' command." ;;
    esac
